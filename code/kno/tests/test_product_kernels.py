...
def test_nn_khatrirao_kernel():
    ...
    grid = quadrature.get_quad_grid(
        quadrature.gauss_legendre_vecs, [3, 5], [-5, -2], [-1, 3]
    )
    _, xq_grid = grid
...
def test_nn_khatrirao_super_res():
    ...
    assert torch.allclose(krao_mat.full_matrix, K_sres, atol=1e-4)

    # now try super res case
    out_quad = quadrature.get_quad_grid(
        quadrature.gauss_legendre_vecs, [6, 10], [-5, -2], [-1, 3]
    )
    ...
    assert torch.allclose(block_to_mat(block_tensor, q, p), K_sres)
