...
from .extern.gpytorch_interpolation import left_interp
import numpy as np
...
def ident_kron_interp(q: int, ind: Tensor, vals: Tensor, u: Tensor) -> Tensor:
    """
    Computes the structured kronecker matrix-matrix product for the
    interpolation integral kernel transform with a SKI interpolant

    Args:
        q (int): size of identity matrix
        ind (Tensor): indices of non-zero elements in interpolation matrix
        vals (Tensor): values of non-zero elements in interpolation matrix
        u (Tensor): khatri-rao product output between K and v

    Returns:
        Tensor: interpolated output
    """
    _, bs = u.shape
    # multiply with identity matrix
    X = u.T
    Z = X.reshape(bs, q, -1).transpose(-2, -1)
    # interpolating
    U = Z.reshape(bs, -1, q)
    U = left_interp(ind, vals, U).transpose(-2, -1).reshape(bs, -1)
    return U.t()
...
def interp_kron_struc_mmprod(
    ind: Tensor, vals: Tensor, K_xx: Union[List, Tensor], K_qp: Tensor, v: Tensor
) -> Tensor:
    """
    Computes the strucuted kronecker matrix-matrix product for the
    interpolation integral kernel transform with a SKI interpolant

    Args:
        ind (Tensor): indices of non-zero elements in interpolation matrix
        vals (Tensor): values of non-zero elements in interpolation matrix
        K_xx (Tensor): grid structured kernel for each dimension
        K_qp (Tensor): task kernel
    v (Tensor): batch last vector of inputs

    Returns:
        Tensor: interpolated kernel vector product
    """
    u = kron_struc_quad_mmprod(K_xx, K_qp, v)
    # reshaping tensor
    (qN, bs) = u.shape
    (q, p) = K_qp.shape
    U = u.reshape(bs, -1, q)
    # interpolating
    U = left_interp(ind, vals, U)
    return U.reshape(-1, bs)
