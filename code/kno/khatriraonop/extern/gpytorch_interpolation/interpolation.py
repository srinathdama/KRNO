...
class Interpolation(object):
    ...
    def interpolate(
        ...
        if lt_min_mask.sum().item():
            first_out_of_range = (
                lt_min_mask.nonzero(as_tuple=False).squeeze(1)[0].item()
            )
            raise RuntimeError(
                (
        ...
        if gt_max_mask.sum().item():
            first_out_of_range = (
                gt_max_mask.nonzero(as_tuple=False).squeeze(1)[0].item()
            )
            raise RuntimeError(
                (
                    "Received data that was out of bounds for the specified grid. "
        ...
        for i in range(num_dim):
            ...
            scaled_dist = lower_pt_rel_dists.unsqueeze(
                -1
            ) + interp_points_flip.unsqueeze(-2)
...
def left_interp(interp_indices, interp_values, rhs):
    ...
    if is_vector:
        ...
        interp_indices_expanded = interp_indices.unsqueeze(-1).expand(
            *batch_shape, num_rows, num_interp, num_columns
        )
        interp_values_expanded = interp_values.unsqueeze(-1).expand(
            *batch_shape, num_rows, num_interp, num_columns
        )
        rhs_expanded = rhs.unsqueeze(-2).expand(
            *batch_shape, num_data, num_interp, num_columns
        )
        res = rhs_expanded.gather(-3, interp_indices_expanded).mul(
            interp_values_expanded
        )
        return res.sum(-2)
...
def left_t_interp(interp_indices, interp_values, rhs, output_dim):
    ...
    interp_indices = interp_indices.expand(
        *batch_shape, *interp_indices.shape[-2:]
    ).contiguous()
    batch_indices = torch.arange(
        0, batch_size, dtype=torch.long, device=values.device
    ).unsqueeze_(1)
    batch_indices = batch_indices.repeat(1, num_data * num_interp)
    column_indices = torch.arange(
        0, num_data * num_interp, dtype=torch.long, device=values.device
    ).unsqueeze_(1)
    column_indices = column_indices.repeat(batch_size, 1)
    summing_matrix_indices = torch.stack(
        [batch_indices.view(-1), interp_indices.view(-1), column_indices.view(-1)], 0
    )
